...
import { listRecentSalesByCharacterSkin } from "@/lib/store";
...
export async function GET(request: NextRequest) {
  ...
  if (!character || !skin) {
    return NextResponse.json(
      { error: "character and skin are required" },
      { status: 400 }
    );
  }
  ...
  const sales = await listRecentSalesByCharacterSkin(character, skin, limit);
